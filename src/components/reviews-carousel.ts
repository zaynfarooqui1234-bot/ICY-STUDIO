export function initReviewsCarousel(): void {
  ...
  const getSlides = () =>
    Array.from(
      carouselContainer.querySelectorAll<HTMLElement>(".reviews-carousel-slide"),
    );
  ...
  const setActiveDot = (activeIndex: number) => {
    dots.forEach((dot, idx) => {
      if (idx === activeIndex) {
        dot.className =
          "reviews-carousel-dot h-[2px] w-9 rounded-full bg-white transition-all duration-300";
      } else {
        dot.className =
          "reviews-carousel-dot h-[2px] w-5 rounded-full bg-gray-500 transition-all duration-300 hover:bg-gray-400";
      }
    });
  };
